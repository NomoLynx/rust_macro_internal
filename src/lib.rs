...
/// generate struct definition from a csv file, the struct field name 
/// is based on the column name in the csv file, and the struct field type inferred from data.
/// there is a static function **Self::to_typed_vec()** generated to return a vector of the struct type,
/// the vector is generated by parsing the csv file and converting each row to the struct type.
#[proc_macro_attribute]
